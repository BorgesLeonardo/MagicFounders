...
const createProject = async (req, res) => {
  const { title, description, goal, image, deadline, category, chavePix } = req.body;
  ...
  if (!validCategories.includes(category)) {
    ...
    logger.warning(`Categoria inválida: ${category}`);
  ...
  try {
    ...
    if (error.message.includes('critico')) {
      logger.critical(`Erro crítico ao criar projeto: ${error.message}`);
    }
...
const supportProject = async (req, res) => {
  ...
  try {
    ...
    if (!project) {
      logger.warning(`Projeto não encontrado: ${projectId}`);
...
const confirmSupport = async (req, res) => {
  ...
  try {
    ...
    logger.critical(`Erro crítico ao confirmar suporte para projeto ID: ${projectId} - ${error.message}`);
...
const updateProject = async (req, res) => {
  ...
  if (!title || !description || !goal || !image || !deadline || !category || !author || !chavePix) {
    logger.warning('Requisição de atualização do projeto faltando campos obrigatórios');
  ...
  try {
    ...
    if (!project) {
      logger.warning(`Projeto não encontrado: ${id}`);
      return res.status(404).json({ message: 'Projeto não encontrado' });
    }

    if (project.creator.toString() !== req.user.id) {
      logger.warning(`Usuário sem permissão para atualizar o projeto ID: ${id}`);
...
const deleteProject = async (req, res) => {
  ...
  try {
    ...
    if (project.creator.toString() !== req.user.id) {
      logger.warning(`Usuário sem permissão para deletar o projeto ID: ${id}`);
